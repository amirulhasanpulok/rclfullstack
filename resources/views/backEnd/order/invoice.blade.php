                        ...
                        <tr>
                            <td style="width: 40%; float: left; padding-top: 15px;">
                                ...
                                <p style="font-size: 14px; color: #222; margin: 20px 0;"><strong>Payment
                                        Method:</strong> <span style="text-transform: uppercase;">{{$order?->payment?$order->payment->payment_method:''}}</span></p>
                            ...
                            <td style="width:60%;float: left;">
                                ...
                                <div class="invoice-bar" style="background: #fff; transform: skew(36deg); width: 72%; margin-left: 182px; padding: 12px 32px; margin-top: 6px;">
                                    <p style="font-size: 15px; color: #222;font-weight:bold; transform: skew(-36deg);
                                     text-align: right; padding-right: 18px">Invoice ID : <strong>#{{$order?->invoice_id}}</strong></p>
                                    <p style="font-size: 15px; color: #222;font-weight:bold; transform: skew(-36deg);
                                     text-align: right; padding-right: 32px">Invoice Date: <strong>{{$order?->created_at->format('d-m-y')}}</strong></span></p>
                                </div>
                    ...
                    <table class="table" style="margin-top: 30px;margin-bottom: 0;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                @endphp
                                <td><img src="{{ asset($data->image->image) }}" alt="" height="100px"
                                        width="100px"></td>
                                <td>{{$value?->product_name ?? " "}} <br> 
                                     @if($value?->product_size) <small>Size:
                                       {{$value?->product_size ?? " "}}</small> 
                                     @endif
                                     @if($value?->product_color)
                                       <small>Color: {{$value?->product_color ?? " "}}</small>
                                     @endif
                                     @if($value?->product_model)
                                       <small>Product Model : {{ $value?->product_model ?? " "}}</small>
                                    @endif
                                    @if($value?->product_weight)
                                      <small>Product Weight : {{ $value?->product_weight ?? " " }}</small>
                                    @endif

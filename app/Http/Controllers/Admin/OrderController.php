...
{
    // If keyword is empty, return no products
    if (empty($request->keyword)) {
        $products = collect(); // empty collection
        return view('backEnd.order.search', compact('products'));
    }

    $products = Product::with('image') // eager load relation
        ->where('status', 1)
        ->where(function ($query) use ($request) {
            $query->where('name', 'LIKE', '%' . $request->keyword . '%')
                  ->orWhere('pro_barcode', 'LIKE', '%' . $request->keyword . '%');
        })
        ->get(); // take all columns (*)
    ...
    //       $pathao_info = Courierapi::where(['status' => 1, 'type' => 'pathao'])->select('id', 'type', 'api_key', 'secret_key', 'url', 'token', 'status')->first();
    //       //dd($pathao_info); //$pathao_info = true;
    //      if ($pathao_info) {
    //         $response = Http::get('https://courier-api-sandbox.pathao.com/aladdin/aladdin/api/v1/city-list');
    //     //   dd($response->json());
    //         $pathaocities = $response->json();
    //         if (!isset($pathao_info->token)) {
    //             $generateToken = Http::withHeaders([
    //                 'Content-Type' => 'application/json'
    //             ])->post('https://api-hermes.pathao.com/aladdin/api/v1/issue-token', [
    //                 'client_id'=>'4QbYyVnbzq',
    //                 'client_secret'=>'ZBZEbCusx9H8vGISsjGSSYevxWV7pXH4e2siPss9',
    //                 'username'=>'mdrabbialom98@gmail.com',
    //                 'password'=>'3698Rabbi@',
    //                 'grant_type'=>'password',
    //             ]);

    //             $generateToken = $generateToken->json();
            
    //             $pathao_info->token = $generateToken['access_token'];
    //             $pathao_info->save();
    //         } else {
    //             $generateToken['access_token'] = $pathao_info->token;
    ...
    public function index($slug, Request $request)
{
    ...
    $pathao_info = Courierapi::where(['status' => 1, 'type' => 'pathao'])
        ->select('id', 'type', 'api_key', 'secret_key', 'url', 'token', 'status')
        ->first();

    if ($pathao_info) {

        // 1. Generate token if empty
        if (empty($pathao_info->token)) {
           $generateToken = Http::withHeaders([
            ...
            if (isset($generateToken['access_token'])) {
                $pathao_info->token = $generateToken['access_token'];
                $pathao_info->save();
            } else {
                \Log::error("Pathao token generation failed", $generateToken);
            }
        }

        // Always use token
        $token = $pathao_info->token;
   ...
   public function pathaocity(Request $request)
{
    ...
    if ($pathao_info) {
        $response = Http::withToken($pathao_info->token)
...
public function pathaozone(Request $request)
{
    ...
    public function order_steadfast($order_id)
    {
        try {
            ...
            if (!$steadfast_info) {
                return redirect()->back()->with(['status' => 'failed', 'message' => 'Steadfast courier info not found.']);
            }

            $consignmentData = [
                'invoice' => $order->invoice_id,
                'recipient_name' => $order->shipping->name ?? 'InboxHat',
                'recipient_phone' => $order->shipping->phone ?? '01323175037',
                'recipient_address' => $order->shipping->address ?? '01323175037',
                'cod_amount' => $order->amount,
            ];

            $response = Http::withHeaders([
                'Api-Key' => "1unf7htd0rnnykwfzw1mcvhnzvvq4l8h",
                'Secret-Key' => "za4twhlbnxk3neokrwqksfg4",
                'Content-Type' => 'application/json',
            ])->post('https://portal.packzy.com/api/v1/create_order', $consignmentData);
            ...
            if ($response->successful()) {
                ...
                $order->update([
                    'order_status' => 5,
                    'courier' => 'steadfast',
                    'tracking_id' => $responseData['consignment']['consignment_id'] ?? null,
                ]);

                return redirect()->back()->with([
                    'status' => 'success',
                    'message' => $responseData['consignment']['tracking_code'] ?? 'Tracking created',
                ]);
            } else {
                return redirect()->back()->with([
                    'status' => 'failed',
    ...
    {
        $data = Order::where(['invoice_id' => $invoice_id])->select('id', 'invoice_id', 'order_status', 'courier', 'tracking_id')->with('orderdetails')->first();
    ...
    public function order_edit($invoice_id)
    {
        ...
        $shippingcharge = ShippingCharge::where('status', 1)->get();
        $order = Order::with('orderdetails')->where('invoice_id', $invoice_id)->first();
        $cartinfo  = Cart::instance('pos_shopping')->destroy();
    ...
    {
        $products = Product::select('id', 'name', 'new_price', 'stock', 'type')
            ->where('status', 1);
        if ($request->keyword) {

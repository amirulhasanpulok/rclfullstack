...
//        $this->middleware('permission:product-delete', ['only' => ['destroy']]);
    ...
    {
        $data['categories'] = Category::where('parent_id', '=', '0')->where('status', 1)->select('id', 'name', 'status')->with('childrenCategories')->get();
        $data['brands'] = Brand::where('status', '1')->select('id', 'name', 'status')->get();
        $data['colors'] = Color::where('status', '1')->get();
        $data['sizes'] = Size::where('status', '1')->get();
        $data['weight'] = Weight::where('status', '1')->get();
        $data['model'] = AdminModel::where('status', '1')->get();
    ...
    public function store(Request $request)
    {
       ...
       // dd($request->all());
        ...
        $input = $request->except(['image', 'product_type', 'files', 'weight', 'model', 'sizes', 'colors', 'purchase_prices', 'old_prices', 'new_prices', 'stocks', 'images']);
        ...
        if ($request->stocks) {
            ...
            $weight      = $request->weight; // ADD THIS
            $model       = $request->model;
            ...
            if (is_array($stocks)) {
                foreach ($stocks as $key => $stock) {
                    ...
                    $variable->weight = $weight ? $weight[$key] : NULL;
                    $variable->model  = $model ? $model[$key] : NULL;
        ...
        FacadesToastr::success('Success', 'Data insert successfully');
    ...
    public function edit($id)
    {
        ...
        $model = AdminModel::where('status', '1')->get();
        $variables = ProductVariable::where('product_id', $id)->get();
        // return $variables;
        return view('backEnd.product.edit', compact('edit_data', 'weight', 'model', 'categories', 'subcategory', 'childcategory', 'brands', 'sizes', 'colors', 'variables'));
    }
    public function price_edit()
    {
        $products = FacadesDB::table('products')->select('id', 'name', 'status', 'old_price', 'new_price', 'stock')->where('status', 1)->get();;
        ...
        if ($request->up_id) {
            ...
            if ($update_ids) {
                foreach ($update_ids as $key => $update_id) {
                    ...
                    $upvariable->weight = $up_weight ? $up_weight[$key] : NULL;
                    $upvariable->model  = $up_model ? $up_model[$key] : NULL;
